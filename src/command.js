...
import createAppcacheStream from "./generate";
...
function watch(globs, options) {
  ...
  return chokidar.watch(globs, {persistent: true, ignoreInitial: true})
...
function validate(globs, options) {
  ...
  if (options.verbose && !options.output) {
    console.error(
      "ERROR: --verbose option should be used together with --output option.");
    hasError = true;
  }
  if (options.watch && !options.output) {
    console.error(
      "ERROR: --watch option should be used together with --output option.");
    hasError = true;
  }
